...
class BottleneckWithCA(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, norm_layer=None):
        ...
        self.conv3 = nn.Conv2d(in_channels=out_channel, out_channels=out_channel * self.expansion,
                               kernel_size=1, stride=1, bias=False)  # unsqueeze channels
        self.bn3 = norm_layer(out_channel * self.expansion)
        self.ca = CoordAtt(inp=out_channel * self.expansion, oup=out_channel * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blockiwithCA,blocks_num, num_classes=1000, include_top=True, norm_layer=None):
        super().__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    """
    resnet_backbone = ResNet(Bottleneck, BottleneckWithCA,[3, 4, 6, 3],
                             include_top=False,
