...
def plot_loss_and_lr(train_loss, learning_rate):
    try:
        ...
        fig.savefig('./logs/loss_and_lr{}.png'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
        plt.close()
...
def plot_map(mAP):
    try:
        ...
        plt.xlim(0, len(mAP))
        plt.legend(loc='best')
        plt.savefig('./logs/mAP{}.png'.format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S")))
        plt.close()
